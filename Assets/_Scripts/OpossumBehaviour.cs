...
public class OpossumBehaviour : MonoBehaviour
{
    ...
    {
        if (los.colliders.Count > 0)
        {
            if (los.collidesWith.gameObject.name == "Player" && los.colliders[0].gameObject.name == "Player")
            {
                return true;
            }
        }
    ...
    private void _FireBullet()
    {
        ...
        if (Time.frameCount % fireDelay == 0 && BulletManager.Instance().HasBullets())
        {
            ...
            var fireDirection = Vector3.Normalize(dest - bulletSpawnPoint.position);

            Debug.Log(fireDirection.ToString());

            BulletManager.Instance().GetBullet(bulletSpawnPoint.position, fireDirection);
        }
    }
